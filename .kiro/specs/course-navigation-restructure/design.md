...
*For any* `ModuleListResponse` returned by `GET /api/courses/{id}/modules`, the `section_count` equals the actual number of Section records with `module_id` matching this module, and `lecture_count` equals the actual number of Lecture records nested under this module's sections. Similarly, *for any* `SectionListResponse`, the `lecture_count` equals the actual number of Lecture records with matching section_id.

**Validates: Requirements 2.3, 2.4, 3.3**

### Property 7: Leading Integer Extraction Correctness

*For any* string starting with digits (possibly zero-padded), `_extract_leading_integer` returns the numeric value with leading zeros stripped. "01" → 1, "1.1" → 1, "10" → 10, "001" → 1. *For any* string not starting with a digit, it returns None.

**Validates: Requirements 7.1, 7.2, 7.4**

### Property 8: Full Numeric Prefix Extraction Correctness

*For any* string starting with a dot-separated numeric prefix, `_extract_full_numeric_prefix` returns a list of integers representing each dot-separated component. *For any* string not starting with digits, it returns an empty list. Trailing dots are ignored.

**Validates: Requirements 8.1, 8.2**

### Property 9: Non-Numeric Items Sort Last

*For any* list containing both items with numeric prefixes and items without numeric prefixes, after sorting with `_grouped_sort_key`, all items without a numeric prefix appear after all items with a numeric prefix.

**Validates: Requirement 6.4**

### Property 10: Endpoint 404 for Invalid IDs

*For any* integer ID that does not correspond to an existing record, the endpoints `/api/courses/{id}/modules`, `/api/modules/{id}/sections`, and `/api/sections/{id}` SHALL return HTTP 404.

**Validates: Requirements 2.2, 3.2, 4.2**

### Property 11: Endpoint Result Ordering

*For any* valid parent ID, the endpoints `/api/courses/{id}/modules` and `/api/modules/{id}/sections` return results ordered by order_index ascending, and `/api/sections/{id}` returns lectures ordered by order_index ascending.

**Validates: Requirements 2.1, 3.1, 4.4**
